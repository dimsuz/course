...
sequence ::
  ...
  List (f a)
  -> f (List a)
sequence afs = foldRight (\ap acc -> lift2 (:.) ap acc) (pure Nil) afs
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA n a = sequence (take n $ produce id a)
